...
```{r echo=FALSE, out.width="60%", fig.cap="The console pane in RStudio is one way to compute with R. Syntax is entered at the R prompt and executed by hitting the <return> or <enter> key."}
knitr::include_graphics("figs/08-console.png")
```
...
```{r echo=FALSE, out.width="60%", fig.cap="LEFT: Arguments are inputted into a function which returns an output. RIGHT: The value 25 is inputted into the square root function which returns the value of 5."}
knitr::include_graphics("figs/09-function-machine.png")
```
...
```{r echo=FALSE, out.width="40%", fig.cap="The enivironment pane shows the objects in the R working environment. It also displays the object's class and gives a preview of the the contents."}
knitr::include_graphics("figs/10-environment.png")
```
...
```{r echo=FALSE, out.width="40%", fig.cap="The object `chili` although still an integer vector, now includes 10 elements."}
knitr::include_graphics("figs/11-environment-chili.png")
```
...
```{r echo=FALSE, out.width="60%", fig.cap="LEFT: Create a new script file using RStudio's `File` menu. RIGHT: Create a new script file by clicking on the `New File` icon in the toolbar."}
knitr::include_graphics("figs/12-new-script-file.png")
```
...
```{r echo=FALSE, out.width="60%", fig.cap="Example script file with comments."}
knitr::include_graphics("figs/13-script-file.png")
```
...
```{r echo=FALSE, out.width="80%", fig.cap="To execute syntax from the script file, highlight the syntax you want to run and then click the `Run` button in the toolbar."}
knitr::include_graphics("figs/14-run-script.png")
```
...
```{r echo=FALSE, out.width="60%", fig.cap="Packages need to be installed and loaded."}
knitr::include_graphics("figs/07-install-vs-load.png")
```
...
```{r echo=FALSE, out.width="40%", fig.cap="The packages tab shows which packages are installed. The list of packages you have installed will likely be different. Checked packages are loaded into memory. In the packages seen here, only the base package is loaded into memory."}
